...
exports.calculateSelfAssessmentScore = calculateSelfAssessmentScore;
exports.calculateHappinessScore = calculateHappinessScore;
// ============================================================================
 ...
 */
function calculateSelfAssessmentScore(assessment) {
    const scoreMap = {
        'none': 12,
        'some': -7,
        'strong': -15
    ...
    return categories.reduce((total, category) => {
        return total + (scoreMap[category] || 0);
    }, 0);
}
// ============================================================================
// MAIN CALCULATION FUNCTION
// ============================================================================
function calculateHappinessScore(questionnaire, selfAssessment, sessions, pahmSessions, stageProgress, dailyNotes = []) {
    ...
    const finalScore = Math.round((currentStateScore * 0.12) + // 12%
...
// ============================================================================
function calculateCurrentStateScore(questionnaire, dailyNotes) {
    let score = 0;
    ...
    score += activityBonusMap[questionnaire.physicalActivity] || 0;
    // Work-Life Balance
    const workLifeBalanceMap = {
        'excellent': 20,
        'good': 12,
        'struggle': -10
    };
    score += workLifeBalanceMap[questionnaire.workLifeBalance] || 0;
    // Stress Response
    const stressResponseMap = {
        'manage_well': 15,
        'usually_manage': 8,
        'overwhelmed': -15
    };
    score += stressResponseMap[questionnaire.stressResponse] || 0;
...
// ============================================================================
function calculateAttachmentScore(selfAssessment) {
...
// ============================================================================
function calculatePAHMScore(questionnaire, pahmSessions) {
    ...
    if (pahmSessions.length > 0) {
        ...
        if (sessionCount >= 100)
            practiceRealization += 35;
        else if (sessionCount >= 50)
            practiceRealization += 30;
        else if (sessionCount >= 25)
            practiceRealization += 25;
        else if (sessionCount >= 15)
            practiceRealization += 20;
        else if (sessionCount >= 10)
            practiceRealization += 15;
        else if (sessionCount >= 5)
            practiceRealization += 10;
        else if (sessionCount >= 1)
            practiceRealization += 5;
        ...
        if (sessionsWithData.length > 0) {
            ...
            const ratedSessions = sessionsWithData.filter(ps => ps.session?.qualityRating && ps.session.qualityRating > 0);
            if (ratedSessions.length > 0) {
                const avgQuality = ratedSessions.reduce((sum, ps) => sum + (ps.session?.qualityRating || 0), 0) / ratedSessions.length;
                if (avgQuality >= 4.5)
                    practiceRealization += 25;
...
// ============================================================================
function calculateEmotionalStabilityScore(questionnaire, sessions) {
    let score = 0;
    ...
    score += stressResponseMap[questionnaire.stressResponse] || 0;
    // Thought Patterns
    const thoughtPatternMap = {
        'peaceful': 15,
        'anxious': -15,
        'neutral': 0
    };
    score += thoughtPatternMap[questionnaire.thoughtPatterns] || 0;
...
// ============================================================================
function calculateMindRecoveryScore(sessions) {
    const completedSessions = sessions.filter(s => s.status === 'completed');
...
// ============================================================================
function calculateEmotionalRegulationScore(questionnaire, sessions) {
    let score = 0;
    // Emotional Awareness × 6.5
    score += (questionnaire.emotionalAwareness || 0) * 6.5;
    // Decision Making
    const decisionMakingMap = {
        'intuitive_mindful': 20,
        'mindful': 12,
        'balanced': 8,
        'overthink': -12
    };
    score += decisionMakingMap[questionnaire.decisionMaking] || 0;
    // Daily Mindfulness
    const dailyMindfulnessMap = {
        'constant': 18,
        'try_to_be': 8,
        'autopilot': -15
    };
    score += dailyMindfulnessMap[questionnaire.mindfulnessInDailyLife] || 0;
...
// ============================================================================
function calculatePracticeConsistencyScore(sessions) {
    const completedSessions = sessions.filter(s => s.status === 'completed');
    if (completedSessions.length === 0)
        return 0;
    let score = 0;
...
// ============================================================================
function calculateSocialConnectionScore(questionnaire) {
    let score = 0;
    // Base score by social connections response
    const socialConnectionMap = {
        'deep_meaningful': 85,
        'few_but_close': 70,
        'good': 55,
        'average': 35,
    ...
    score += socialConnectionMap[questionnaire.socialConnections] || 0;
...
// ============================================================================
function determineUserLevel(score) {
    if (score >= 80)
        return 'Enlightened Seeker';
    if (score >= 65)
        return 'Advanced Seeker';
    if (score >= 50)
        return 'Progressing Seeker';
    if (score >= 35)
        return 'Awakening Seeker';
    if (score >= 20)
...
function calculateStreakDays(sessions) {
    ...
    const daysSinceLastSession = Math.floor((currentDate.getTime() - mostRecentSession.getTime()) / (1000 * 60 * 60 * 24));
