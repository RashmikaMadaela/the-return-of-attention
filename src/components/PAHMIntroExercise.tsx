...
interface PAHMIntroExerciseProps {
  onComplete?: () => void
}

export default function PAHMIntroExercise({ onComplete }: PAHMIntroExerciseProps) {
  ...
  const handleComplete = async () => {
    ...
    try {
      const response = await fetch('/api/pahm-intro/complete', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok || !data.success) {
        setError(data.message || 'Failed to mark PAHM intro as complete')
      ...
      if (onComplete) {
        ...
        // Default behavior: redirect to stage 1 with refresh
        router.push('/stage-1?refresh=true')
      }
    } catch (err) {
      console.error('Error completing PAHM intro:', err)
      setError('Failed to mark PAHM intro as complete')
      setIsCompleting(false)
    }
  }
      ...
      <div className="p-4 sm:p-8 pt-20 sm:pt-24 pb-8">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="bg-white rounded-xl p-4 sm:p-6 md:p-8 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h3 className="font-bold mb-1 text-sm sm:text-base">Enhanced Self-Awareness</h3>
                  <p className="text-xs sm:text-sm text-gray-700">Discover your mind's habitual patterns and tendencies.</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3">
                <div className="text-2xl">😌</div>
                <div>
                  <h3 className="font-bold mb-1 text-sm sm:text-base">Reduced Mental Chatter</h3>
                  <p className="text-xs sm:text-sm text-gray-700">Gradually decrease time spent in rumination and worry.</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3">
                <div className="text-2xl">💪</div>
                <div>
                  <h3 className="font-bold mb-1 text-sm sm:text-base">Stronger Attention</h3>
                  <p className="text-xs sm:text-sm text-gray-700">Build your capacity to stay present for longer periods.</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3">
                <div className="text-2xl">😊</div>
                <div>
                  <h3 className="font-bold mb-1 text-sm sm:text-base">Genuine Happiness</h3>
                  <p className="text-xs sm:text-sm text-gray-700">Experience happiness independent of external circumstances.</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <button
              onClick={handleBack}
              className="bg-gray-600 hover:bg-gray-700 text-white px-6 sm:px-8 py-3 rounded-lg font-semibold transition-colors text-sm sm:text-base"
            >
              Back
            </button>
            <button
              ...
              className={`bg-green-600 hover:bg-green-700 text-white px-6 sm:px-8 py-3 rounded-lg font-semibold transition-colors text-sm sm:text-base ${
